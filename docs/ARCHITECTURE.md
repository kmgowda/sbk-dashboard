...
- POSIX process sessions/signals versus Windows process groups and `Ctrl+Break` for the optional detached launcher.
...
The repository launchers reuse the selected environment and keep a PID-plus-creation-time identity for their small
log-draining supervisor. That supervisor bounds control-plane logs and forwards shutdown to the normal application
lifecycle; it does not replace native-service ownership or the production recommendation to use a host service
manager. A separate minimal parent-death watcher observes the launcher's PID and creation time. If the launcher is
killed, the watcher signals the isolated dashboard process group, waits for normal cleanup, and then forcefully
removes only captured descendants that exceed the shutdown bound.
