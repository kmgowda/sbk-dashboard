...
The repository launchers reuse the selected environment and keep a mode-plus-PID-plus-creation-time identity for the
single script-started instance. The default launcher runs the control plane in its own foreground process so logs and
signals stay attached to the console. On Windows, a thin console-attached parent places the foreground application
in a dedicated process group, forwards `Ctrl+Break`, and has a parent-death watcher; this lets both interactive
interrupts and the separate stop script use graceful application shutdown without creating an orphan. The optional
background launcher uses a small log-draining supervisor that
