...
under `SBK_DASHBOARD_HOME/app/<version>/<platform>/<source-fingerprint>`. Python 3.10+ with venv is therefore the
only source-bootstrap prerequisite; package-index access can still be required on the first start. An exclusive,
