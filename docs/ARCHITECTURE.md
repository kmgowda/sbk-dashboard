...
The repository launchers reuse an active venv/Conda environment, otherwise use or create the project `.venv`, and
install the local application plus missing runtime dependencies before launcher acquisition. Python 3.10+ with venv
support is therefore the only initial host prerequisite; package-index access can still be required for missing
dependencies. They keep one port-plus-mode-plus-PID-plus-creation-time identity per script-started instance. Help
