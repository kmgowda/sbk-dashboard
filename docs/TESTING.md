...
those operating systems because native executables cannot be meaningfully launched through Linux simulation. The
cross-platform unit workflow uses Python 3.14 on the explicit Apple Silicon `macos-15` runner and Windows 2022,
while the Linux quality job protects the Python 3.10 minimum. This split catches standard-library removals that
remain available as deprecated aliases on older Python versions. The explicit macOS runner avoids the moving
`macos-latest` label so runner OS and architecture changes cannot silently alter the required check. The job asserts
that macOS reports `arm64` and invokes the installed CLI before running the unit suite.
...
comparison UIDs, immutable comparison-policy/selection validation, duplicate-before-capacity error ordering,
configured and bounded comparison-ID API validation, complete cached-policy refresh, two-or-more single-target time lanes and comparison cache,
...
uninstalling or modifying the host), confirm the exact release archive is verified below `cache/releases`, and then
repeat with network access disabled. Exercise foreground, background, `stop -port`, stop-all, `--help`, argument
values containing spaces, and `repair`. This validation must use Linux, macOS, and Windows native hosts for each
published standalone architecture; Linux unit tests do not constitute native macOS/Windows validation.
